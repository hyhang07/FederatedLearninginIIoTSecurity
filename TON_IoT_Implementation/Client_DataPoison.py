# Client_DataPoison.py (Malicious - Data Poisoning)
import os
import time
import argparse
import numpy as np
...
NUM_CLASSES = 9
PRUNING_PERCENTAGE = 0.80  # Send more parameters to maximize attack impact
...
def aggressive_label_poisoning(y_train, num_classes, poison_ratio=0.8):
    """Flips labels strategically to generate antagonistic gradients."""
    print(f"\n--- [ATTACK INITIATED] Target: Data Poisoning (Ratio: {poison_ratio*100}%) ---")
    np.random.seed(42)  # Coordinated attack across clients
    ...
    for idx in poison_indices:
        ...
        # Targeted Shift: Always misclassify as a different, specific class
        wrong_class = (original_class + 2) % num_classes
        y_poisoned[idx] = wrong_class
        
    print(f"   - Successfully poisoned {num_poison} data labels.")
    return y_poisoned

def train_and_upload(args):
    print(f"\n{'='*60}")
    print(f"[MALICIOUS CLIENT - DATA POISON] Client {args.client_id} | Zone: {args.zone_name}")
    print(f"{'='*60}")
    ...
    # --> INJECT POISON HERE <--
    y_train_poisoned = aggressive_label_poisoning(y_train, NUM_CLASSES, poison_ratio=0.8)
    ...
    print("\n--- Training on POISONED Local Data ---")
    callbacks =[EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True)]
    local_model.fit(X_train, y_train_poisoned, epochs=25, batch_size=256, validation_split=0.2, verbose=1, callbacks=callbacks)
    
    print(f"\n--- Compressing Malicious Update (Top-{PRUNING_PERCENTAGE*100:.0f}%) ---")
    trained_weights = local_model.get_weights()
    update_delta =[(new - old) for new, old in zip(trained_weights, initial_weights)]
    
    # Add extra chaos noise to bypass simple detection
    noisy_delta =[]
    for layer in update_delta:
        noise = np.random.normal(0, np.std(layer) * 0.5, layer.shape)
        noisy_delta.append(layer + noise)
    
    all_deltas_flat = np.concatenate([np.abs(layer.flatten()) for layer in noisy_delta])
    k = int(len(all_deltas_flat) * PRUNING_PERCENTAGE)
    threshold = np.sort(all_deltas_flat)[-max(1, k)]
    
    sparse_update =[np.where(np.abs(layer) >= threshold, layer, 0.0) for layer in noisy_delta]
